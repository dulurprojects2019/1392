...
class Iconbar extends CI_Controller {
	...
	public function add(){
		...
		$this->form_validation->set_rules('icb_image', 'Slide Home Page', 'callback_validate_image');
    ...
    if ((!isset($_FILES['icb_image'])) || $_FILES['icb_image']['size'] == 0) {
        ...
        if(filesize($_FILES['icb_image']['tmp_name']) > 3000000) {
    ...
    if (isset($_FILES['icb_image']) && $_FILES['icb_image']['size'] != 0) {
        ...
        if(filesize($_FILES['icb_image']['tmp_name']) > 3000000) {
            $this->form_validation->set_message('validate_image_edit', 'The Image file size shoud not exceed 3MB!');
            $check = FALSE;
        }
        if(!in_array($extension, $allowedExts)) {
